...
/**
 * Muteer (misschien) een organisme
 * @param organism (in/out) Het organisme om (misschien) te muteren
 */
static void mutate(Organism* organism)
{
...
void breed(Population* population)
{
    ...
    for(unsigned int i = 0; i < offspringSize; ++i)
    {
        ...
        // Muteer het kind (eventueel)
        mutate(child);

        // Bereken de fitness-waarde van het kind
        fitness(child);
